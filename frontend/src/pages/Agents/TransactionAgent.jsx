    ...
    <div>
      ...
      <div style={{ position: 'relative', height: 80, border: '1px solid var(--color-border)', borderRadius: 8, overflow: 'hidden', background: 'var(--color-surface-2)' }}>
        ...
        <div style={{ position: 'absolute', left: 0, right: 0, top: '8%', borderTop: '1.5px dashed #A32D2D', zIndex: 2 }}>
          <span style={{ position: 'absolute', right: 4, top: -10, fontSize: 9, color: '#E82AAE', fontWeight: 600 }}>LKR 5M threshold</span>
        </div>
        {/* Bars */}
        {txns.map((t, i) => {
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {[
              ...
              { label: 'STR Eligible', value: data.surveillance_summary.str_eligible_count, sub: 'CBSL FIU filing required', color: '#E82AAE', tooltip: 'Accounts meeting CBSL FIU criteria for a Suspicious Transaction Report under the Financial Transactions Reporting Act. NTB must file within 5 working days of identification.', alert: 'File within 5 working days' },
              { label: 'High Risk Accounts', value: data.surveillance_summary.high_risk_accounts, sub: `${data.surveillance_summary.flagged_transactions} total flagged transactions`, color: '#26EA9F', tooltip: 'Accounts with velocity multiples > 3× their 90-day baseline, or counterparty concentration > 70% (hub-and-spoke pattern), or participating in a detected structuring cluster.' },
          ...
          <div className="agent-grid">
            ...
            <div style={{ background:'#A32D2D06', border:`1px solid #A32D2D22`, borderRadius:10, overflow:'hidden' }}>
              <div style={{ padding:'12px 16px', display:'flex', gap:10, alignItems:'flex-start' }}>
                <div style={{ fontSize:10, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.08em', padding:'3px 9px', borderRadius:5, background:'#E82AAE', color:'white', flexShrink:0, marginTop:2 }}>
                  ADVERSE
                </div>
                <div style={{ fontSize:12, color:'#E82AAE', lineHeight:1.7 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', borderTop:`1px solid #A32D2D18` }}>
                {[['Population tested','284,719 transactions (100%)'],['Period covered','FY 2025 (Jan–Dec)'],['Materiality threshold','LKR 5M STR threshold; velocity anomalies ≥3× 90-day baseline'],['Model limitations','Benford&#39;s Law requires sufficient volume per account; new accounts (&lt;90 days) excluded from velocity baseline']].map(([k,v],i)=>(
                  <div key={i} style={{ padding:'7px 16px', borderRight:i%2===0?`1px solid #A32D2D12`:'none', borderBottom:i<2?`1px solid #A32D2D12`:'none' }}>
                    <div style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', color:'#E82AAE', opacity:0.65, marginBottom:2 }}>{k}</div>
                    <div style={{ fontSize:11, color:'#E82AAE', lineHeight:1.5 }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {data.benford_analysis.deviation_detected && (
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', background: '#FCEBEB', color: '#E82AAE', borderRadius: 4 }}>ANOMALY DETECTED</span>
                )}
              </div>
                ...
                <ResponsiveContainer width="100%" height={190}>
                  <BarChart data={BENFORD_DATA} margin={{ top: 4, right: 8, bottom: 4, left: -24 }}>
                    ...
                    <Bar dataKey="actual" name="actual" radius={[2, 2, 0, 0]}>
                      {BENFORD_DATA.map((d, i) => <Cell key={i} fill={d.digit === '4' ? '#E82AAE' : Math.abs(d.deviation) > 2 ? '#26EA9F' : COLOR} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                <div style={{ padding: '0 8px 12px' }}>
                  <InlineExplainer color="#E82AAE" text={`Digit "4" appears at 18.3% vs expected 9.7% — an 89% excess. This means transactions in the LKR 4.0M–4.99M range are being deliberately created just below the LKR 5M STR threshold. Chi-squared p-value: 0.003 (highly significant — not random).`} />
                </div>
              </div>
            </div>
                ...
                {activeTab === 'structuring' && (data.structuring_clusters || []).map((cl, i) => (
                  <div key={i} style={{ padding: '14px 16px', borderBottom: '1px solid var(--color-border)' }}>
                    <div style={{ display: 'flex', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
                      ...
                      {cl.str_eligible && <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', background: '#E82AAE', color: 'white', borderRadius: 4 }}>STR</span>}
                ...
                {activeTab === 'velocity' && (data.velocity_anomalies || []).map((v, i) => (
                  <div key={i} style={{ display: 'flex', gap: 14, padding: '12px 16px', borderBottom: '1px solid var(--color-border)', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginTop: 6 }}>
                        ...
                        <div style={{ height: 8, borderRadius: 4, background: 'var(--color-surface-2)', overflow: 'hidden', position: 'relative' }}>
                          ...
                          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '100%', background: v.velocity_multiple >= 10 ? '#E82AAE' : '#26EA9F', opacity: 0.4 }} />
                        </div>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: v.velocity_multiple >= 10 ? '#E82AAE' : '#26EA9F', fontVariantNumeric: 'tabular-nums' }}>{v.velocity_multiple}×</div>
                    ...
                    {(data.str_queue || []).map((str, i) => (
                      <div key={i} style={{ padding: '14px 16px', borderBottom: '1px solid var(--color-border)', background: str.urgency === 'immediate' ? '#FEF0F0' : 'transparent' }}>
                        <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', background: str.urgency === 'immediate' ? '#E82AAE' : '#3A5A3A', color: 'white', borderRadius: 4 }}>{str.urgency.replace('_', ' ')}</span>
                          <span style={{ marginLeft: 'auto', fontSize: 14, fontWeight: 800, color: '#E82AAE', fontVariantNumeric: 'tabular-nums' }}>LKR {(str.amount_lkr/1e6).toFixed(0)}M</span>
                        </div>
