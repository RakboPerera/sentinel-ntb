...
const SECTOR_COLORS = { 'Real Estate':'#C41E3A','Construction':'#4A6070','Tourism':'#0BBF7A','Manufacturing':'#1F2937','Consumer':'#26EA9F','Other':'#8A8A85' };
        ...
        return (
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
              ...
              <MetricSpotlight value={`LKR ${((ps.estimated_ecl_understatement_lkr||0)/1e6).toFixed(0)}M`} label="ECL Understatement" sub="Stage misclassification" color="#C41E3A" trend="Provision gap" trendDir="up" />
                ...
                <div style={{ padding:'16px' }}>
                  <ResponsiveContainer width="100%" height={220}>
                    <ScatterChart margin={{ top:10, right:20, bottom:10, left:10 }}>
                      ...
                      <ReferenceLine x={0.65} stroke="#C41E3A" strokeDasharray="4 3" label={{ value:'Flag', fontSize:9, fill:'#C41E3A' }} />
                  ...
                  <div style={{ marginTop:12, display:'flex', gap:8, flexWrap:'wrap' }}>
                    {[['0.65–0.80','High anomaly',COLOR,'12'],['0.80–1.00','Critical outlier','#C41E3A','4']].map(([r,l,c,n])=>(
                ...
                <div className="agent-panel">
                  ...
                  <div style={{ padding:'16px' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
                      <ComparisonSplit leftLabel="Current Tier 1 CAR" leftValue={`${ci.current_tier1_car||19.06}%`} leftColor="#0BBF7A" rightLabel="Corrected CAR" rightValue={`${ci.corrected_tier1_car||18.59}%`} rightColor="#C41E3A" note={`−${ci.car_impact_bps||47}bps · ${(ci.car_impact_bps||47)>=50?'⚠ Exceeds CBSL 50bps notification threshold':'Below notification threshold'}`} />
                    </div>
                    <HeatStrip value={ci.additional_rwa_lkr||4.2e9} max={20e9} color="#C41E3A" label="Additional RWA" sublabel="Basel III risk-weighted impact of corrected staging" format={v=>`LKR ${(v/1e9).toFixed(1)}Bn`} />
                  </div>
                </div>
                  ...
                  <div style={{ maxHeight:240, overflowY:'auto' }}>
                    ...
                    {(data.flagged_loans||[]).filter(l=>l.management_staging_conflict).slice(0,6).map((l,i)=>(
                      <div key={i} style={{ display:'grid', gridTemplateColumns:'80px 60px 60px 1fr', padding:'9px 14px', borderBottom:'1px solid var(--color-border)', background: i%2===0?'transparent':'var(--color-surface-2)', alignItems:'center' }}>
                        ...
                        <span style={{ fontSize:11, padding:'2px 7px', background:'#FCEEF1', color:'#C41E3A', borderRadius:4, fontWeight:700, width:'fit-content' }}>S{l.required_stage}</span>
