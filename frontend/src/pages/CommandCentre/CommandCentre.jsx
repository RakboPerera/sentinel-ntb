...
const SCORE_ZONES = [
  ...
  { from: 0.85, to: 1.0,  label: 'Critical', color: '#C41E3A' },
];
...
function AlertDrawer({ alert, onClose }) {
  ...
  const severityColor = isCritical ? '#C41E3A' : '#4A6070';
          ...
          {activeTab === 'signals' && (
            <>
              ...
              <div style={{ display: 'flex', gap: 12, fontSize: 10, color: 'var(--color-text-3)', paddingTop: 4, borderTop: '1px solid var(--color-border)' }}>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><div style={{ width: 16, height: 3, background: '#C41E3A', borderRadius: 1 }} />Critical &gt;0.8</span>
              </div>
            </>
          )}
          ...
          {activeTab === 'workflow' && (
            <>
              ...
              {alert.sla && (() => {
                ...
                const slaColor = isBreached ? '#C41E3A' : pct >= 75 ? '#4A6070' : '#16A34A';
                return (
              ...
              <div style={{ padding:'12px 14px', background:'var(--color-surface-2)', border:'1px solid var(--color-border)', borderRadius:8 }}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {[
                    { label:'Confirmed fraud — open case', color:'#C41E3A', bg:'#FEF0F0' },
                  ...
                  <div>
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:8 }}>
                      {['Confirmed fraud','False positive','Inconclusive','STR-Eligible'].map(d => {
                        const col = d==='Confirmed fraud'?'#C41E3A':d==='False positive'?'#16A34A':d==='STR-Eligible'?'#4A6070':'#185FA5';
...
const ALL_FINDINGS = [
  { id:'F-001', agent:'Insider Risk', agentColor:'#1F2937', finding:'STF-1847 — All 6 insider dimensions breached simultaneously. Risk score 94/100.', severity:'critical', exposure:'LKR 387M', branch:'BR-14', daysOpen:22, status:'Under Investigation', owner:'R. Wijeratne', type:'Insider Fraud', path:'/agents/insider-risk' },
  ...
  { id:'F-007', agent:'Transaction Surveillance', agentColor:'#4A6070', finding:'NTB-0841-X — 15 CEFT transfers in 22 min, LKR 4.6M–4.95M each. Structuring score 0.94.', severity:'high', exposure:'LKR 71M', branch:'BR-72', daysOpen:22, status:'STR Pending', owner:'MLCO', type:'AML', path:'/agents/transaction' },
  ...
  { id:'F-015', agent:'Insider Risk', agentColor:'#1F2937', finding:'STF-2341 (BR-23) — score 71/100. Override concentration 52%, off-hours approvals 18%.', severity:'medium', exposure:'—', branch:'BR-23', daysOpen:22, status:'Open', owner:'Unassigned', type:'Insider Risk', path:'/agents/insider-risk' },
  ...
  { id:'F-017', agent:'Transaction Surveillance', agentColor:'#4A6070', finding:'NTB-2209-F velocity anomaly — 34 txns vs baseline 4 (8.5× above normal). LKR 18.4M.', severity:'medium', exposure:'LKR 18M', branch:'BR-72', daysOpen:22, status:'Open', owner:'AML Analyst', type:'AML', path:'/agents/transaction' },
];
...
function FindingsRegisterView({ navigate }) {
  ...
  function ageColor(d) { return d > 60 ? '#C41E3A' : d > 30 ? '#4A6070' : '#16A34A'; }
  function sevColor(s) { return { critical: '#C41E3A', high: '#4A6070', medium: '#185FA5' }[s] || '#6b6963'; }
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
        {[
          ...
          { label:'Critical', val:critCount, sub:'Immediate action required', color:'#C41E3A' },
          { label:'Overdue >30 days', val:overdueCount, sub:'Passed standard resolution SLA', color:'#C41E3A' },
        ].map((s,i) => (
      ...
      <div className="agent-panel" style={{ overflow:'hidden' }}>
            ...
            <tbody>
              {filtered.map((f,i) => (
                ...
                >
                  ...
                  <td style={{ padding:'8px 10px', fontWeight:700, color:'#C41E3A', whiteSpace:'nowrap' }}>{f.exposure}</td>
        ...
        <div style={{ padding:'10px 16px', borderTop:'1px solid var(--color-border)', background:'var(--color-surface-2)', fontSize:10, color:'var(--color-text-3)', display:'flex', justifyContent:'space-between' }}>
          <span>Age: <span style={{ color:'#16A34A' }}>■</span> &lt;30d &nbsp;<span style={{ color:'#4A6070' }}>■</span> 30–60d &nbsp;<span style={{ color:'#C41E3A' }}>■</span> &gt;60d — findings requiring action</span>
...
function RegulatoryView({ navigate }) {
  function urgencyColor(u) { return { critical:'#C41E3A', high:'#4A6070', medium:'#185FA5', filed:'#16A34A', low:'#6b6963' }[u] || '#6b6963'; }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
        {[
          { label:'STR Filings Pending', val:pendingSTRs, sub:'Under FTRA — 5 working day limit', color: pendingSTRs > 0 ? '#C41E3A' : '#16A34A' },
          { label:'STR Overdue', val:overdueSTRs, sub:'Deadline passed — immediate action', color:'#C41E3A' },
            ...
            <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
              <div style={{ flexShrink:0, textAlign:'center', minWidth:60 }}>
                {str.status === 'Filed' ? (
                  ...
                  <>
                    <div style={{ fontSize:20, fontWeight:900, color:str.daysRemaining < 0 ? '#C41E3A' : str.daysRemaining <= 1 ? '#4A6070' : '#185FA5', lineHeight:1 }}>
              ...
              {str.status !== 'Filed' && (
                <button onClick={() => navigate('/cases', { state: { caseId: str.case } })} style={{ padding:'6px 14px', background:'#C41E3A', color:'white', border:'none', borderRadius:8, fontSize:12, fontWeight:700, cursor:'pointer', flexShrink:0, whiteSpace:'nowrap' }}>
                  File STR →
                </button>
              )}
            </div>
      ...
      <div className="agent-panel">
        ...
        <div style={{ padding:'4px 0' }}>
          {KYC_REMEDIATION.map((r, i) => {
            ...
            const pc = { critical:'#C41E3A', high:'#4A6070', medium:'#185FA5', low:'#16A34A' }[r.priority];
            return (
                ...
                <div style={{ padding: '16px' }}>
                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    <LineChart data={executiveData.regulatory_trend} margin={{ top: 4, right: 16, bottom: 4, left: -16 }}>
                      ...
                      <ReferenceLine yAxisId="left" y={11.5} stroke="#C41E3A" strokeDasharray="4 3" label={{ value: 'CAR min 11.5%', fontSize: 9, fill: '#C41E3A' }} />
                      <ReferenceLine yAxisId="right" y={100} stroke="#C41E3A" strokeDasharray="4 3" label={{ value: 'LCR min 100%', fontSize: 9, fill: '#C41E3A' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                {[
                  ...
                  { label: 'Fraud Detected', value: `LKR ${(executiveData.ai_roi.fraud_detected_lkr/1e9).toFixed(1)} Bn`, sub: 'Total value of confirmed fraud cases', color: '#C41E3A' },
            ...
            <div className="agent-panel">
              ...
              <div style={{ padding: '16px' }}>
                ...
                {executiveData.risk_appetite.map((r, i) => {
                  ...
                  const color = r.status === 'green' ? '#3B6D11' : r.status === 'amber' ? '#26EA9F' : '#C41E3A';
                  return (
                    <div key={i} style={{ marginBottom: 16 }}>
                      ...
                      <div style={{ height: 8, borderRadius: 4, background: 'var(--color-surface-2)', overflow: 'hidden', position: 'relative' }}>
                        ...
                        {r.limit > 0 && !higherGood && (
                          <div style={{ position: 'absolute', left: '100%', top: -2, bottom: -2, width: 2, background: '#C41E3A', transform: 'translateX(-2px)' }} />
                        )}
                      </div>
                      {r.status === 'red' && !higherGood && (
                        <div style={{ fontSize: 10, color: '#C41E3A', marginTop: 3, fontWeight: 600 }}>⚠ Limit breached — {Math.round((r.actual/(r.limit||1)-1)*100)}% above limit</div>
                      )}
                      {r.status === 'amber' && (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={branchData} margin={{ top: 0, right: 8, bottom: 0, left: -20 }}>
                  ...
                  <Bar dataKey="score" radius={[4, 4, 0, 0]} label={{ position: 'top', fontSize: 10, fill: '#6b6963' }}>
                    {branchData.map((d, i) => (
                      <Cell key={i} fill={d.score < 50 ? '#C41E3A' : d.score < 65 ? '#26EA9F' : '#3B6D11'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div style={{ display: 'flex', gap: 14, justifyContent: 'center', marginTop: 8, fontSize: 11, color: 'var(--color-text-2)' }}>
                {[['#C41E3A','Critical <50'],['#26EA9F','Amber 50–65'],['#3B6D11','Good >65']].map(([c,l]) => (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5 }}>
                      <div style={{ flex: 1, height: 3, borderRadius: 2, background: 'var(--color-surface-2)', overflow: 'hidden' }}>
                        <div style={{ width: `${alert.anomalyScore * 100}%`, height: '100%', background: alert.anomalyScore >= 0.85 ? '#C41E3A' : '#26EA9F', borderRadius: 2 }} />
                      </div>
