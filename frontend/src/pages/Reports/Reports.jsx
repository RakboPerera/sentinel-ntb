              ...
              {(typeof section.content === 'string' ? section.content : String(section.content)).split('\n').filter(l => l.trim()).map((line, li) => {
                ...
                if (isPair) {
                  ...
                  return <div key={li} style={{ display:'grid', gridTemplateColumns:'200px 1fr', gap:8, padding:'7px 16px', borderBottom:'1px solid var(--color-border)', alignItems:'baseline' }}>
                    ...
                    <span style={{ fontSize:12, color:'var(--color-text)', lineHeight:1.5, fontWeight: val.includes('CRITICAL') || val.includes('CONFIRMED') ? 700 : 400, color: val.includes('CRITICAL') ? '#DC2626' : val.includes('HIGH') ? '#D97706' : val.includes('EFFECTIVE') ? '#16A34A' : 'var(--color-text)' }}>{val}</span>
                  </div>;
                }
