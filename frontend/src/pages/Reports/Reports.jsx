...
function buildReport(reportId) {
  ...
  const sections = {
    ...
    credit: [
      { title:'AUDIT OPINION — CREDIT RISK', type:'mini_opinion', text:'The SLFRS 9 staging control environment is PARTIALLY EFFECTIVE. While the portfolio Stage 3 ratio of 0.91% appears healthy, full-population AI analysis identifies 34 loans (LKR 1.10 Bn) that are predicted to be misstaged — potentially understating ECL provisions. The concurrence of override approval patterns at BR-14 with staging anomalies indicates the control weakness may be partly intentional.', classification:'PARTIALLY EFFECTIVE', classColor:'#4A6070' },
      ...
      {sections.map((section, i) => (
        <div key={i} style={{ marginBottom:28 }}>
          ...
          {section.type !== 'opinion' && section.type !== 'mini_opinion' && section.content && (
            <div style={{ background:'var(--color-surface-2)', borderRadius:10, border:'1px solid var(--color-border)', overflow:'hidden' }}>
              {(typeof section.content === 'string' ? section.content : String(section.content)).split('\n').filter(l => l.trim()).map((line, li) => {
                const isHeader = line.startsWith('##');
                const isSub = line.startsWith('#');
                const isDivider = line === '---';
                const isPair = line.includes(':') && !line.startsWith(' ') && line.indexOf(':') < 40;
                if (isDivider) return <div key={li} style={{ borderBottom:'1px solid var(--color-border)', margin:'4px 0' }} />;
                if (isHeader) return <div key={li} style={{ padding:'10px 16px 6px', fontSize:11, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.07em', color:report.color, background:`${report.color}08`, borderBottom:'1px solid var(--color-border)' }}>{line.replace(/^#+\s*/,'')}</div>;
                if (isSub) return <div key={li} style={{ padding:'8px 16px 4px', fontSize:11, fontWeight:700, color:'var(--color-text-2)', textTransform:'uppercase', letterSpacing:'0.05em' }}>{line.replace(/^#+\s*/,'')}</div>;
                if (isPair) {
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
                ...
                <tbody>
                  {section.actions.map((a,ai) => {
                    const sc = a.status==='Resolved'?'#16A34A':a.status.includes('Progress')||a.status.includes('filed')?'#185FA5':a.status==='In Progress'?'#185FA5':'#4A6070';
                    return (
