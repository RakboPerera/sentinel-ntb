...
export default function OrchestratorPanel({ signals, agentColor }) {
  ...
  return (
    <div className="agent-panel" style={{ borderColor: 'var(--color-purple-light)' }}>
      <div className="agent-panel-header" style={{ background: 'var(--color-purple-light)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span className="agent-panel-title" style={{ color: 'var(--color-purple)' }}>Orchestrator Signals</span>
        </div>
        <span style={{ fontSize: 11, color: 'var(--color-purple)', fontWeight: 500 }}>
      ...
      <div className="agent-panel-body">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {signals.map((sig, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 14px', background: 'var(--color-surface-2)', borderRadius: 8, border: '1px solid var(--color-border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: 6, background: sig.severity === 'critical' ? 'var(--color-red-light)' : 'var(--color-amber-light)', flexShrink: 0 }}>
                <AlertTriangle size={13} style={{ color: sig.severity === 'critical' ? 'var(--color-red)' : 'var(--color-amber)' }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 600, padding: '1px 6px', borderRadius: 4, background: sig.severity === 'critical' ? 'var(--color-red-light)' : 'var(--color-amber-light)', color: sig.severity === 'critical' ? 'var(--color-red)' : 'var(--color-amber)' }}>{sig.severity}</span>
                </div>
        ...
        {orchResult && orchResult.correlations && (
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-purple)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {orchResult.correlations.map((corr, i) => (
              <div key={i} style={{ padding: '14px 16px', background: 'linear-gradient(135deg, var(--color-purple-light) 0%, var(--color-red-light) 100%)', border: '1px solid rgba(83,74,183,0.2)', borderRadius: 10, marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-purple)' }}>Severity {(corr.combined_severity * 100).toFixed(0)}%</span>
